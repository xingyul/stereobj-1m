...
The data can be downloaded [here](https://www.dropbox.com/sh/b1e5xuzysyxqg0a/AAANEt13l8zWSxcv7IkIzVEwa?dl=0). You can find the stereo images, 6D pose annotations, pre-generated instance masks and dataset split information. The annotations for test set are held out. To obtain the performance on test set, please use EvalAI.
...
We implementated [KeyPose](https://arxiv.org/abs/1912.02805) as a baseline method. The code for the baseline is in `baseline_keypose/`. Please refer to `baseline_keypose/README.md` for more details on how to use the code.

## Evaluation Script and File format
The command scripts for launching 6D pose evaluation is located in `evaluation/`.
Please refer to `evaluation/README.md` for more details on how to use the evaluation script.
